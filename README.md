...
|---|---|---|---|
| Judge cases | 70 | `tests/harness_manifest.json` | Accepted / malformed / unparsed / incomplete_proof / incorrect on curated fixtures (incl. FALSE_CERT_TOO_LARGE; accepted certificates at exactly the documented 100 KB / 20 KB size caps, rejections one byte over; accepted FALSE certificates cover finite tables and infinite carriers — `Nat` and a submission-defined inductive tree type) |
...
-- The pipeline passes `judge.lean_timeout_seconds = 300` from `pipeline/config.json`, and the raw `judge/verify.py` default is also 300 s, so a bare call and the pipeline agree. `export LEAN_TIMEOUT_SECONDS=<n>` overrides for a direct call; to change the cap globally, edit `pipeline/config.json`.
